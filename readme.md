...
이메일, 비밀번호 입력 -> 백엔드API에서 확인 -> 사용자 생성

백엔드에서 API에서 확인 -> 인증 토큰 생성(문자열,백엔드에서 생성되고 서명되며 위조할 수 없다) -> 모바일로 전송
...
기본적으로 firebase는 데이터베이스를 잠그지 않으면 누구나 액세스 가능.
고유의 API를 구축하려는 경우, 누구든 액세스 가능한 URL 외에도 특정한 라우트가 필요하다.
인증된 사용자만 액세스 가능한 특정 엔드포인트가 필요하다. 우리는 사용자를 로그인하지만 현재 백엔드에서는
인증된 사용자와 데이터 사이에 연결이 존재하지 않는다. 이건 백엔드 API기 무상태성이라서 그렇다.
Firebase는 사용자의 로그인 정보를 저장하지 않는다. 그렇기 때문에 토큰을 통해서 액세스 권한을 부여하거나
보호된 엔드포인트를 액세스할 권한을 얻는다. 이것은 firebase의 읽기 규칙을 변경하면 된다.

만약 token을 필요로 하는 요청의 경우 엔드포인트에 auth를 추가하면 된다.

```
axios.get(`https://authentication-28e29-default-rtdb.firebaseio.com/message.json?auth=${token}`)
```
